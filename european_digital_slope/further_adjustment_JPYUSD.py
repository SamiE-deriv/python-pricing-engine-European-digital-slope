    ...
    while current_date <= end_date:
        try:
            ...
            vol_surface = build_vol_surface(
                current_date, market_type="forex", symbol="JPYUSD"
            )

            # Get spot price from data
            data_path = Path("european_digital_slope/data/forex/frxUSDJPY")
...
def create_plots(
    ...
    fig.suptitle(
        f"{month} 2024 - JPY/USD {option_type.upper()} Options Analysis", fontsize=16
    )
    ...
    ax1.set_title("JPY/USD Spot Price and ATM Volatility")
