    ...
    while current_date <= end_date:
        try:
            ...
            vol_surface = build_vol_surface(
                current_date, market_type="forex", symbol="GBPUSD"
            )

            # Get spot price from data
            data_path = Path("european_digital_slope/data/forex/frxGBPUSD")
            date_str = current_date.strftime("%Y%m%d")
            filename = f"frxGBPUSD_{date_str}.csv"
...
def create_plots(
    ...
    fig.suptitle(
        f"{month} 2024 - GBP/USD {option_type.upper()} Options Analysis", fontsize=16
    )
    ...
    ax1.set_title("GBP/USD Spot Price and ATM Volatility")
